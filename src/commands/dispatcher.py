...
COMMANDS = check_mods(
    ...
    ["event", "top_espers", (handle_top_espers, phrases("top_espers_help"))],
    ["likes", "top_likes", (handle_top_likes, phrases("top_likes_help"))],
    ["likes", "like", (handle_like, phrases("like_help"))],
    ["roll_title", "rename", (prepare_game, phrases("rename_help"))],
    ["roll_title", "reg", (start, phrases("reg_help"))],
    [
        "drink_or_not",
        "drink_or_not",
        (handle_drink_or_not(drinks), phrases("drink_or_not_help")),
    ],
    [
        "how_much",
        "how_much_esper",
        (handle_how_much(espers), phrases("how_much_esper_help")),
    ],
    ["event", "players", (send_players, phrases("players_help"))],
    ["event", "play", (play, phrases("play_help"))],
)
...
handlers = check_mods(
    ["core", handle_cmd, handler(cmd_ok, commands=list(COMMANDS.keys()))],
    ["core", handle_unknown, handler(cmd_no_ok)],
    ["long_message", handle_long(long_message), handler(is_long_message)],
    [
        "reply",
        handle_text_to_sticker(text_to_sticker),
        handler(message_ok(text_to_sticker)),
    ],
    ["reply", handle_text_to_text(text_to_text), handler(message_ok(text_to_text))],
    [
        "reply",
        handle_stickers(sticker_to_sticker),
        handler(sticker_ok(sticker_to_sticker), content_types=["sticker"]),
    ],
    [
        "captcha",
        handle_new_user,
    ...
    ["captcha", handle_verify_captcha, handler(is_captcha_user)],
)

inline_handlers = {handle_inline_help(COMMANDS): {lambda query: query.query == ""}}

query_check_data = lambda data: lambda call: chat_ok(
    call.message
) and call.data.startswith(data)
query_handlers = check_mods(
    ["event", handle_title_change_attempt, {"func": query_check_data("number_")}],
    ["captcha", handle_captcha_button_press, {"func": query_check_data("captcha_")}],
)
