...
class SqliteLikesRepository(SqliteRepository, LikesRepository):
    ...
    def like_and_count(self, chat_id: int, from_user_id: int, to_user_id: int) -> int:
        def _tx(conn: Connection):
            self._likesDao.save(conn, chat_id, from_user_id, to_user_id)
            return self._likesDao.count_by_chat_and_to_user(conn, chat_id, to_user_id)
        ...
        def _tx(conn: Connection):
            return Leaderboard(
                ...
                rows=[
                    ...
                    for row in self._likesDao.get_leaderboard(conn, chat_id, limit)
                ],
            )
